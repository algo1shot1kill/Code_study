...
---
### 아이디어
1. 단어 길이별 정방향/역방향 트라이 생성
2. pro??? 로 들어오면 해당 길이의 정방향 트라이 탐색해서 와일드 카드 '?' 전 노드가 가지는 단어 수 저장
3. ???pro 로 들어오면 해당 길이의 orp???로 변경한 후 역방향 트라이 탐색해서 와일드 카드 '?' 전 노드가 가지는 단어 수 저장

---
### 유의점
1. 쿼리가 모두 ?로 이루어져 있을 때, ex) [?????] 해당 길이를 가지는 트라이의 루트 노드가 가지는 단어의 개수(cnt)를 answer배열에 저장해주어야 함!!!
   안그러면 search 메서드 실행시 if(c == '?') break; 에 의해 탐색 바로 종료 => [0] 출력 됨.
